...
const CATEGORY_ICONS: Record<string, React.ElementType> = {
  DAILY_CASUAL: (props: any) => <span {...props}>👕</span>,
  OVERSIZE_STYLE: (props: any) => <span {...props}>👚</span>,
  COUPLE_GROUP: (props: any) => <span {...props}>👥</span>,
};
...
function LookbookCard({ photo }: { photo: LookbookPhoto }) {
  ...
  const CategoryIcon = CATEGORY_ICONS[photo.category] || (() => null);

  async function handleDelete() {
    if (!confirm(`Delete this lookbook photo? This cannot be undone.`)) return;
    setDeleting(true);
    const res = await deleteLookbookPhoto(photo.id);
    if (res.success) {
      toast.success("Photo deleted");
      // Optionally trigger a refresh (we could use router.refresh or mutate)
      ...
      <div className="relative aspect-[3/4] bg-[var(--color-surface)] overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
          <Link
            ...
            className="inline-flex items-center gap-1.5 bg-white text-[var(--color-text)] px-3 py-1.5 rounded-[var(--radius-btn)] text-xs font-medium hover:bg-[var(--color-surface)] transition-colors shadow-sm"
          >
            <Edit size={12} />
            Edit
          </Link>
          <button
            ...
            disabled={deleting}
            className="inline-flex items-center gap-1.5 bg-red-500 text-white px-3 py-1.5 rounded-[var(--radius-btn)] text-xs font-medium hover:bg-red-600 transition-colors disabled:opacity-50 shadow-sm"
          >
            <Trash2 size={12} />
            {deleting ? "..." : "Delete"}
          </button>
